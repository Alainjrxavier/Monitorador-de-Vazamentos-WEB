...
from app.forms import EmailSearchForm, RegistrationForm, LoginForm, AddEmailForm
from app.models import User, MonitoredEmail
...
def index():
    ...
    if form.validate_on_submit():
        ...
        if results.get('breaches') or results.get('pastes'):
            app.logger.warning(f"{log_message} VAZAMENTO ENCONTRADO.")
        elif results.get('error') and "429" not in str(results.get('error')):
            app.logger.error(f"{log_message} Erro crítico na consulta. Erro: {results.get('error')}")
        else:
...
def dashboard():
    ...
    for email_obj in monitored_emails_objects:
        breach_info = check_pwned_email(email_obj.email)
        dashboard_data.append({
            'db_entry': email_obj,
            'breach_info': breach_info
        })
        # A LINHA ABAIXO É A MUDANÇA PRINCIPAL NESTA FUNÇÃO
        time.sleep(1.6) # Pausa para não exceder o limite da API no loop
